...
def test_build_clarity_brief_maps_each_field_from_the_documented_source():
    ...
    assert brief.situation == state.surface_complaint
    # key_insights = primary_problem + risks + opportunities, in that order
    assert brief.key_insights == [
        _JUDGMENT.primary_problem,
        *_JUDGMENT.risks,
        *_JUDGMENT.opportunities,
    ]
    assert brief.current_direction == _PLANNER.desired_outcome
    assert brief.remaining_unknowns == _JUDGMENT.open_unknowns

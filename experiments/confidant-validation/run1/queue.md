# Confidant Architecture Validation -- Test Queue (Run 1, ARCHIVED)

**Status: ARCHIVED as of 2026-07-07.** This is Run 1's queue, stopped
after D01 failed twice under the new OpenRouter-only config (see
`engine/decisions.md` "Ollama removed, OpenRouter-only"). C01-R05 ran
under the old openrouter->ollama fallback config; D01 was attempted but
never completed a valid run (both attempts failed at Judgment before a
fallback existed to catch them) and is NOT scored in `log.md`. Run 2
(`../queue.md`, `../log.md`) restarts the full 30-test suite from C01
under the OpenRouter-only config, so the two runs' results are never
mixed in one dataset. Kept here in full, not deleted, per this project's
standing discipline of preserving completed work as a visible record.

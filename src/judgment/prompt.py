...
Sometimes the user message additionally includes a Retrieved Context
section -- behavioral patterns and cross-session themes already
computed, offline, about this same person from PAST Journeys (see
...
def build_messages(world_state_json: str, stagnation_signals: list, retrieved_context: str = ""):
    """
    ...
    `retrieved_context` (added for Retrieval, see engine/decisions.md
    and src/retrieval/engine.py): the already-formatted output of
    build_retrieved_context, or "" for a Journey with no cross-session
    history yet. Passed in pre-resolved, same contract as
    src/planner/prompt.py's/src/response/prompt.py's own `mode`
    parameter -- this module stays a pure function of strings, it
    doesn't need to know src/retrieval/engine.py exists.
    """
    ...
    if retrieved_context:
        content += f"\n\nRetrieved Context (from past Journeys, not generated by you):\n{retrieved_context}"
